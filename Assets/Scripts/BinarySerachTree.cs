...
public class BinarySerachTree<TKey, TValue> : IDictionary<TKey, TValue> where TKey : IComparable<TKey>
{
    ...
    public ICollection<TKey> Keys => InOrderTraversal(root).Select(kvp => kvp.Key).ToList();

    public ICollection<TValue> Values => InOrderTraversal(root).Select(kvp => kvp.Value).ToList();
    ...
    protected virtual TreeNode<TKey, TValue> Add(TreeNode<TKey, TValue> node, TKey key, TValue value)
    {
        ...
        {
            throw new ArgumentException($"키 {node.Key} 중복");
        }
    ...
    public IEnumerable<KeyValuePair<TKey, TValue>> InOrderTraversal(TreeNode<TKey, TValue> node)
    {
        if (node != null)
        {
    ...
    public IEnumerable<KeyValuePair<TKey, TValue>> PreOrderTraversal(TreeNode<TKey, TValue> node)
    {
        if (node != null)
        {
            yield return new KeyValuePair<TKey, TValue>(node.Key, node.Value);
    ...
    public IEnumerable<KeyValuePair<TKey, TValue>> PostOrderTraversal(TreeNode<TKey, TValue> node)
    {
        if (node != null)
        {
    ...
    public IEnumerable<KeyValuePair<TKey, TValue>> LevelOrderTraversal(TreeNode<TKey, TValue> node)
    {
    ...
    {
        node.Height = 1 + (int)MathF.Max(Height(node.Left), Height(node.Right));
    }
